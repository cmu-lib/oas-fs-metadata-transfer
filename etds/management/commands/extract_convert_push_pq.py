...
from edts.models import pqAttempt,pqFsAttempt
...
class Command(BaseCommand):
	...
	with open('ozzycats/ANZSRC.csv', newline='') as csvfile:
		reader = csv.reader(csvfile)
		for row in reader:
			department_categories.append(row[1])  # 0 for the first column
	...
	def xml_to_json(file):
		with open(file) as xml_file:
			# make a dict
			return xmltodict.parse(xml_file.read())[p+'submission']

	def handle_failures(e):
	...
	def upload_parts(file_info,file_location):
	...
	def get_token():
		...
		token_file = open('fs_token.txt','w')
		token_file.write(str(datetime.datetime.now().isoformat())+'\n')
	...
	def is_token():
		...
		try:
			...
			token_time = datetime.datetime.fromisoformat(lines[0].strip())
			token = lines[1].strip()
			token_file.close()

			if (datetime.datetime.now() - token_time).total_seconds() < 3500: #tokens last an hour, so if it's been less than 3500 seconds just use it.
				print('reusing token')
				return(token)
			else:
	...
	def process_name(author,is_author=True):
		name = author[p+'name'][p+'fname']+" "+str('' if author[p+'name'][p+'middle'] is None else author[p+'name'][p+'middle'])+ " " +author[p+'name'][p+'surname']
		first_name = author[p+"name"][p+'fname']
		last_name = author[p+"name"][p+'surname']
		if is_author:
			orcid_id = author[p+"orcid"]
		else:
			orcid_id = ""
	...
	def convert_pq_to_fs(data_dict):
		figshare_json = {'authors':[],'keywords':[],"defined_type":"thesis","group_id":secrets["figshare_group_id_dev"]}
		# authors first
		authors_data = data_dict[p+'authorship']
		if isinstance(authors_data,list):
			for x in authors_data:
				figshare_json['authors'].append(self.process_name(x[p+'author']))
		else:
			# print(authors_data)
			figshare_json['authors'].append(self.process_name(authors_data[p+'author']))

		#basic info like title, abstract, date
		title = data_dict[p+"description"][p+'title']
		# print(title)
		published_date = data_dict[p+"description"][p+'dates'][p+'accept_date']
		published_date= datetime.datetime.strptime(published_date, "%d/%m/%Y").strftime("%Y-%m-%d")
		# print(published_date)
		# exit()

		advisors_data = data_dict[p+"description"][p+"advisor"]
		advisors_str = ""
		...
		if isinstance(advisors_data,list):
			...
			# figshare_json['advisors'].append(self.process_name(advisors_data[p+'advisor'],False))


		#content
		if isinstance(data_dict[p+"content"][p+'abstract'][p+'para'],list):
			description = ' '.join(data_dict[p+"content"][p+'abstract'][p+'para'])
		else:
			description = data_dict[p+"content"][p+'abstract'][p+'para']#[0:1999]

		#embargo
		if data_dict["@embargo_code"] != "0":
			figshare_json['is_embargoed'] = True
			embargo_date = datetime.datetime.strptime(data_dict[p+'restriction'][p+'sales_restriction']['@remove'], "%m/%d/%Y").strftime("%Y-%m-%d")
		...
		if data_dict[p+"description"][p+'degree'] not in degree_type_dev:
			we_dont_have_degree = open('we_dont_have_degree.txt','a')
			we_dont_have_degree.write(data_dict[p+"description"][p+'degree']+'\n')
			we_dont_have_degree.close()	
			degree_type = ""
		else:
			degree_type = data_dict[p+"description"][p+'degree']

		# department name here.
		dept_name = data_dict[p+"description"][p+'institution'][p+"inst_contact"]
		# print(dept_name)
		if dept_name not in department_categories:
			# dept_name = ""
			# note what we don't have 
			we_dont_have_program = open('we_dont_have_program.txt','a')
			we_dont_have_program.write(dept_name+'\n')
			we_dont_have_program.close()
			# add this cat into keywords
			figshare_json['keywords'].append(dept_name)

		# keywords
		figshare_json['keywords']+=re.split(r',\s*',data_dict[p+"description"][p+'categorization'][p+'keyword']) if data_dict[p+"description"][p+'categorization'][p+'keyword'] is not None else []
		print(figshare_json['keywords'])
		print("^^keywords^^")
		#pretty slick, no?
		...
		#             {
		#                 "DISS_cat_code": "0771",
		#                 "DISS_cat_desc": "Robotics"

		#cats
		first_cat_loop = True
		cat_desc = data_dict[p+"description"][p+'categorization'][p+'category']
		categories = []
		print(cat_desc)
		if isinstance(cat_desc,list):
			print('there is more than one department catlist')
			...
			for cat in cat_desc:
				if cat[p+'cat_desc'] in department_categories:
					if first_cat_loop:
						categories.append(cat[p+'cat_code'])
						first_cat_loop = False
					else:
						categories.append(cat[p+'cat_code'])
				else:
					we_dont_have_program = open('we_dont_have_program.txt','a')
					we_dont_have_program.write(cat[p+'cat_desc']+'\n')
					we_dont_have_program.close()
					categories = []

		elif isinstance(cat_desc,dict):
			print('it is singlular')
			print(cat_desc[p+'cat_desc'])
			print('and '+cat_desc[p+'cat_code'])
			if cat_desc[p+'cat_desc'] not in department_categories:
				print(" this singular category not in options!")
				categories = []
				we_dont_have_program = open('we_dont_have_program.txt','a')
				we_dont_have_program.write(cat_desc[p+'cat_desc']+'\n')
				we_dont_have_program.close()
			else:
				categories.append(cat_desc[p+'cat_code'])
	...
	def rm_user_and_add_file(article_id,file_location): # figshare automatically adds myself as a user to articles. 
		headers = {
			'Content-Type': 'application/json',
		}
		params = {
	...
	#LET'S GO
	for z_thesis in ziplist:
		if '.zip' not in z_thesis: # its not a zip.
			continue
		print('working on '+z_thesis)
		data_dict = {}
		# check if there are files first
		are_there_files = os.listdir('zippies/working')
		if len(are_there_files)>0: # if there are then kill them
			for f in are_there_files:
				os.remove('zippies/working/'+f)
		# loading the temp.zip and creating a zip object

		with ZipFile("./zippies/"+z_thesis, 'r') as z_t:

			# Extracting all the members of the zip
			# into a specific location.
			z_t.extractall(
				path="./zippies/working")
		the_files = os.listdir('zippies/working')
		a_file = the_files[0].split('.')
		if a_file[1]=='pdf':
			# this is pdf use the other file (1)
			xml_file = file_location+the_files[1]
			data_dict= self.xml_to_json(xml_file)
			upload_file = file_location+the_files[0]
		else:
			xml_file = file_location+the_files[0]
			data_dict= self.xml_to_json(xml_file)
			upload_file = file_location+the_files[1]
		print(the_files[1])
		...
		open('pq_jsons/'+data_dict[p+"description"][p+'title'].replace("/","").replace(" ","_")+".json" ,'w').write(json.dumps(data_dict,indent=4))
		figshare_json = self.convert_pq_to_fs(data_dict)
		# save converted json in case there's an issue downstream
		open('jsons/'+figshare_json['title'].replace("/","").replace(" ","_")+".json" ,'w').write(json.dumps(figshare_json,indent=4))
		# give it a whirl and post.
		# continue
		...
		try: # this should be expanded. there are many other places this can fail. hhhhhhhhhhh
			...
			if response['code'] in ['UnknownException', 'BadRequest', 'UnprocessableEntity']:
				...
				self.rm_user_and_add_file(article_id,upload_file)
				os.remove(xml_file)
