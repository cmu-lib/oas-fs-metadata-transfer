	...
	def handle(self, *args, **options):
		if options['zip'] is not None:
			for z_thesis in options['zip']:	
				...
				# exit()
				data_dict = {}
				# check if there are files first in the working directory
				...
				#delete the existing pqAttempt and pqFsAttempt if they exist for this zip.
				try:
					pqa = pqAttempt.objects.get(zip_title=z_thesis)
					pqfs = pqFsAttempt.objects.get(pqa_fk=pqa)
					pqa.delete()
					pqfs.delete()
				except pqAttempt.DoesNotExist:
					pass
				supplemental_files = []
				for f in the_files:
					print(f)
